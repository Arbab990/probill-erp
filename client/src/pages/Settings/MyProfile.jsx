...
const MyProfile = () => {
    ...
    const handlePasswordChange = async () => {
        ...
        setSaving(true);
        try {
            await settingsService.updateProfile({ currentPassword: pwForm.currentPassword, newPassword: pwForm.newPassword });
        ...
        <div className="page-container">
            ...
            <div className="max-w-xl space-y-5">
                ...
                <Card>
                    <div className="flex items-center gap-4 mb-5 pb-5 border-b border-dark-border">
                        <div className="w-16 h-16 rounded-xl bg-primary/20 border border-primary/30 flex items-center justify-center">
                            <span className="text-2xl font-bold text-primary-light">{user?.name?.charAt(0)?.toUpperCase()}</span>
                        </div>
                        <div>
                    ...
                    <div className="space-y-4">
                        <Input label="Full Name" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="Your name" />
                    ...
                    <Button className="mt-4" loading={saving} onClick={handleProfileSave}>Save Profile</Button>
                </Card>
                ...
                <Card>
                    ...
                    <div className="space-y-4">
                        <Input label="Current Password" type="password" value={pwForm.currentPassword} onChange={e => setPwForm(p => ({ ...p, currentPassword: e.target.value }))} placeholder="••••••••" />
                        ...
                        <Input label="Confirm New Password" type="password" value={pwForm.confirmPassword} onChange={e => setPwForm(p => ({ ...p, confirmPassword: e.target.value }))} placeholder="Repeat new password" />
                    </div>
                    <Button variant="secondary" className="mt-4" loading={saving} onClick={handlePasswordChange}>Change Password</Button>
                </Card>
            </div>
        </div>
    );
